# jkit Iteration 4: Discoverability — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement `publish-contract` — the skill that generates a 4-level progressive disclosure service contract so other Claude instances in other repos can route to the right service, domain, and API without loading everything at once.

**Architecture:** `publish-contract` uses `bin/jkit skel` for controller scanning, smart-doc for static OpenAPI generation (no app restart needed), and a structured 7-question interview to fill contract metadata. Output goes to `docs/contracts/{service-name}/` inside the microservice repo. The skill runs inside a microservice project, not the jkit plugin repo.

**Tech Stack:** Claude Code plugin skill system (SKILL.md), YAML frontmatter, Markdown, smart-doc Maven plugin, yq/python3 for JSON→YAML conversion
...
| File | Action |
|------|--------|
| `skills/publish-contract/SKILL.md` | Create (4-level contract: scan, Javadoc check, domain mapping, interview, smart-doc, write output) |

---

### Task 1: Create skills/publish-contract/SKILL.md

**Files:**
- Create: `skills/publish-contract/SKILL.md`

Two HARD-GATEs: one before generating any output (domain mapping approval) and one before overwriting an existing contract. The 4-level disclosure map: Level 1+2 = `overview.md`, Level 3 = `domains/{name}.md`, Level 4 = `reference/contract.yaml` (grepped on demand).

Note: this is the Iteration 4 version. Iteration 5 replaces `overview.md` with `SKILL.md` and adds a GitHub push pipeline. Do not add iter5 content here.

- [ ] **Step 1: Create directory**

```bash
mkdir -p skills/publish-contract
```

- [ ] **Step 2: Write skills/publish-contract/SKILL.md**
...
Expected: at least `2` (domain approval gate + overwrite approval gate)

---

### Task 2: Commit Iteration 4

- [ ] **Step 1: Stage and commit**

```bash
git add skills/publish-contract/SKILL.md
git commit -m "feat: add publish-contract skill"
```
